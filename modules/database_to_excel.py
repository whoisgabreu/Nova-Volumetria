...
def compare_shipment_name():
    excel_dir = os.path.join(os.path.expanduser('~'), 'OneDrive', 'OneDrive - J&T EXPRESS BRAZIL LTDA', 'PowerBi Volumetria SC')
    wb = openpyxl.load_workbook(filename = f"{excel_dir}/data_nome_rota.xlsx", read_only = True)
    df_header = ["dest_site", "shipment_name"]
    df = pd.DataFrame(wb.active.values, columns = df_header)
